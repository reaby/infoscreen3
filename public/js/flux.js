...
/**
 * Contains additional changes for infoScreen3
 * Free to use under the MIT license.
 * http://www.opensource.org/licenses/mit-license.php
 */
...
(function ($) {
    ...
    flux.slider.prototype = {
        ...
        setSize(w, h) {
            ...
            this.imageContainer.css({"width": w + "px", "height": h + "px"});
            this.image1.css({"width": w + "px", "height": h + "px"});
            this.image2.css({"width": w + "px", "height": h + "px"});
            this.setupImages();
        },
        ...
        showTempImage: function (id, trans, opts) {
            let i = 0;
            for (var img of this.images) {
                if (img.class === "temp" && img.id === id) {
                    this.showImage(i, trans, opts);
                    break;
                }
                i++;
...
(function ($) {
    ...
    flux.transition.prototype = {
        ...
        finished: function () {
            ...
            this.slider.isTransition = false;
    ...
    flux.transition_grid = function (fluxslider, opts) {
        return new flux.transition(fluxslider, $.extend({
            ...
            forceSquare: false,
            setup: function () {
                var imgWidth = this.slider.options.width,
                    imgHeight = this.slider.options.height;
                ...
                var colRemainder = imgWidth - (this.options.columns * colWidth),
                    colAddPerLoop = Math.floor(colRemainder / this.options.columns),
        ...
        return new flux.transition_grid(fluxslider, $.extend({
            ...
            renderTile: function (elem, colIndex, rowIndex, colWidth, rowHeight, leftOffset, topOffset) {
                $(elem).css({
                    ...
                    'background-size': this.slider.options.width + "px " + this.slider.options.height + "px",
                }).css3({
            ...
            execute: function () {
                ...
                var height = this.slider.options.height;
        ...
        return new flux.transition_grid(fluxslider, $.extend({
            ...
            renderTile: function (elem, colIndex, rowIndex, colWidth, rowHeight, leftOffset, topOffset) {
                var bar = $('<div class="bar-' + colIndex + '"></div>').css({
                        width: colWidth + 'px',
                        height: this.slider.options.height + "px",
                        position: 'absolute',
                        top: '0px',
                        left: '0px',
                        'z-index': 200,
...
(function ($) {
    flux.transitions.concentric = function (fluxslider, opts) {
        return new flux.transition(fluxslider, $.extend({
            ...
            alternate: false,
            setup: function () {
                var w = this.slider.options.width,
                    h = this.slider.options.height,
...
(function ($) {
    flux.transitions.cube = function (fluxslider, opts) {
        return new flux.transition(fluxslider, $.extend({
            ...
            perspective: 1000,
            setup: function () {
                var width = this.slider.options.width;
                var height = this.slider.options.height;
                ...
                var css = {
                    height: this.slider.options.height + "px",
                    width: this.slider.options.width + "px",
                    position: 'absolute',
                    top: '0px',
                    left: '0px'
                };
...
(function ($) {
    flux.transitions.tiles3dprev = function (fluxslider, opts) {
        return new flux.transition_grid(fluxslider, $.extend({
            requires3d: true,
            forceSquare: false,
            columns: 6,
            ...
            renderTile: function (elem, colIndex, rowIndex, colWidth, rowHeight, leftOffset, topOffset) {
                ...
                $(elem).css({
                    ...
                    'transition-timing-function': 'ease-out',
                    'transition-property': 'all',
                    'transition-delay': ((this.options.columns - 1 - colIndex) * this.options.delayBetweenBarsX + (this.options.rows - 1 - rowIndex) * this.options.delayBetweenBarsY) + 'ms',
                    'transform-style': 'preserve-3d'
                }).append(tile).append(tile2);
            },
            execute: function () {
                ...
                // Get notified when the last transition has completed
                tiles.first().transitionEnd(function (event) {
                    _this.slider.image2.show();
                    _this.finished();
                });
            ...
            setup: function () {
                var tab = $('<div class="tab"></div>').css({
                    ...
                    front = $('<div></div>').appendTo(tab).css({
                        'background-image': this.slider.image1.css('background-image'),
                        'background-position': (this.options.direction == 'left' ? '-' + (this.slider.options.width / 2) : 0) + 'px 0',
                        width: "100%",
                        height: "100%",
                        position: 'absolute',
                        top: '0',
                    ...
                    back = $('<div></div>').appendTo(tab).css({
                        'background-image': this.slider.image2.css('background-image'),
                        'background-size': this.slider.options.width + "px " + this.slider.options.height + "px",
                        'background-position': (this.options.direction == 'left' ? 0 : '-' + (this.slider.options.width / 2)) + 'px 0',
                        width: '100%',
                        height: '100%',
                        position: 'absolute',
                        top: '0',
...
(function ($) {
    flux.transitions.slide = function (fluxslider, opts) {
        return new flux.transition(fluxslider, $.extend({
            ...
            execute: function () {
                var _this = this,
                    delta = this.slider.options.width;
